...
import { Post, store, useStore } from "@/lib/store";
...
export default function PostCard({ post }: { post: Post }) {
  ...
  const distance = currentUser 
    ? calculateDistance(currentUser.lat, currentUser.lng, post.lat, post.lng).toFixed(1)
    : '??';
  ...
  const handleHelp = () => {
    // In a real app, this would notify the owner
  ...
  return (
    <Card className="overflow-hidden border-none shadow-sm hover:shadow-md transition-shadow">
      <CardHeader className="p-4 flex flex-row items-center gap-3">
        <Avatar className="w-10 h-10" style={{ backgroundColor: post.avatarCor }}>
          <AvatarFallback className="bg-transparent text-white font-bold">{post.avatarLetra}</AvatarFallback>
        </Avatar>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className="font-bold text-sm truncate">{post.autor}</span>
            <Badge variant="outline" className={`text-[10px] py-0 px-1.5 h-4 font-normal ${typeColors[post.tipo]}`}>
              {post.tipo}
            </Badge>
          </div>
          <div className="flex items-center gap-3 text-[10px] text-muted-foreground">
            <span className="flex items-center gap-1"><MapPin className="w-3 h-3" /> {post.zona}, {distance}km</span>
            <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> {formatDistanceToNow(post.ts, { addSuffix: true, locale: pt })}</span>
          </div>
        </div>
      </CardHeader>
      ...
      <CardFooter className="p-2 bg-muted/30 flex items-center justify-between border-t border-muted/50">
        <div className="flex items-center gap-1">
          <Button variant="ghost" size="sm" className="h-8 text-[11px] gap-1.5 px-2">
            <MessageSquare className="w-4 h-4" /> {post.comentariosCount}
          </Button>
          <Button variant="ghost" size="sm" className="h-8 text-[11px] gap-1.5 px-2">
            <CheckCircle2 className="w-4 h-4" /> {post.candidatosCount} candidatos
          </Button>
        </div>

        {post.uid !== currentUser?.uid && post.status === 'aberto' && (
